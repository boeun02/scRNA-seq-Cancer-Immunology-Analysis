...
![ISG15_score_by_Cancer_Type]
(docs/figures/phase2b_subtype_score_violon_isg15.png)

### 결과 분석
각 암종의 macrophage population은 평균적으로 어떤 TAM marker program이 강한지 알아보기 위해 해당 plot들을 그려보았다.  
- C1QC: KIDNEY, OV-FTC, PAAD에서는 높게 나타나지만 세가지 cell중에서 음수로 측정된 점수들이 유독 낮았다. 특정 암종에서만 제한적인 발현량을 가지는 것으로 예상된다.
- SPP1: 가장 score가 높은 암이 PAAD로 0.695정도이다. 별로 눈에 띌만한 결과는 아니다.
- ISG15: LYM만이 1점대 이상으로 높게 나타났다. 나머지는 거의 비슷하게 낮게 분포한다.  
...
### 결과 분석
암종별 macrophage population에서 C1QC-, SPP1-, ISG16-associated marker program score가 양성으로 관찰된 조합을 Upset plot으로 요약하였다. 각 막대는 특정 score-positive 조합을 보인 암종의 수를 의미한다.
